...
public class LooperBase extends ImageButton {
    private int soundToPlay=0;
    private int buttonNumber=0;
    int playingId=0;
    boolean playing=false;
    String baseSound;
    public LooperBase (final Context context, final AttributeSet attrs) {
        super(context, attrs);
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if(!playing) {
               ...
               /* switch(Pad.baseLinePlayingIs)
                ...
                // }
                Pad.mSoundPoolHelper.stop(Pad.getBaseLinePlayingId());
                Pad.turnLightOff(Pad.getBaseLinePlayingNumber());
                Pad.mSoundPoolHelper.setLoop(true);

                playingId = Pad.mSoundPoolHelper.play(soundToPlay);
                Pad.setBaseLinePlaying(baseSound,playingId,buttonNumber);
