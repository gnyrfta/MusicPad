...
public class LooperDrums extends ImageButton {
    private int soundToPlay=0;
    private int buttonNumber=0;
    int playingId=0;
    boolean playing=false;
    String drumSound;
    public LooperDrums (final Context context, final AttributeSet attrs) {
        super(context, attrs);
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if(!playing) {
               ...
               /* switch(Pad.baseLinePlayingIs)
                ...
                // }
                Pad.mSoundPoolHelper.stop(Pad.getDrumsPlayingId());
                Pad.turnLightOff(Pad.getDrumsPlayingNumber());
                Pad.mSoundPoolHelper.setLoop(true);

                playingId = Pad.mSoundPoolHelper.play(soundToPlay);
                Pad.setDrumsPlaying(drumSound,playingId,buttonNumber);
    ...
    public void setDrumSound(String baseSound) {
        this.drumSound = baseSound;
    }
}
