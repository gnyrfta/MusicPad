...
public class LooperChords extends ImageButton {
    private int soundToPlay=0;
    private int buttonNumber=0;
    int playingId=0;
    boolean playing=false;
    String chordSound;
    public LooperChords (final Context context, final AttributeSet attrs) {
        super(context, attrs);
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if(!playing) {
               ...
               /* switch(Pad.baseLinePlayingIs)
                ...
                // }
                Pad.mSoundPoolHelper.stop(Pad.getChordsPlayingId());
                Pad.turnLightOff(Pad.getChordsPlayingNumber());
                Pad.mSoundPoolHelper.setLoop(true);

                playingId = Pad.mSoundPoolHelper.play(soundToPlay);
                Pad.setChordsPlaying(chordSound,playingId,buttonNumber);
    ...
    public void setChordSound(String baseSound) {
        this.chordSound = baseSound;
    }
}
